...
impl CoverageCollector {
    ...
    pub fn initialize_coverage(&self, selected_tests: &[String]) -> Result<()> {
        ...
        info!(
            "Initializing incremental coverage for {} tests",
            selected_tests.len()
        );
    ...
    pub fn collect_coverage(&self, test_results: &[String]) -> Result<CoverageMap> {
        ...
        debug!(
            "Collecting coverage data from {} test results",
            test_results.len()
        );
    ...
    pub fn merge_coverage_maps(
        &self,
        base: &CoverageMap,
        delta: &CoverageMap,
    ) -> Result<CoverageMap> {
        debug!(
            "Merging coverage maps: base={} files, delta={} files",
            base.files.len(),
            delta.files.len()
        );
            ...
            match merged.files.get(file_path) {
                Some(base_coverage) => {
                    ...
                    if base_coverage.digest == delta_coverage.digest {
                        let merged_coverage =
                            self.merge_file_coverage(base_coverage, delta_coverage)?;
                        merged.files.insert(file_path.clone(), merged_coverage);
                    } else {
                        // File changed, use delta coverage (more recent)
                        warn!(
                            "File {} changed (digest mismatch), using new coverage",
                            file_path
                        );
                        merged
                            .files
                            .insert(file_path.clone(), delta_coverage.clone());
                    }
                }
                None => {
    ...
    pub fn generate_full_report(&self, coverage_map: &CoverageMap) -> Result<()> {
        ...
        info!(
            "Generating full coverage report with {} files",
            coverage_map.files.len()
        );
    ...
    pub fn save_coverage_map(&self, coverage_map: &CoverageMap) -> Result<()> {
        ...
        std::fs::write(&cache_file, json).context("Failed to write coverage map to cache")?;
    ...
    pub fn load_coverage_map(&self) -> Result<Option<CoverageMap>> {
        ...
        let coverage_map: CoverageMap =
            serde_json::from_str(&json).context("Failed to deserialize coverage map")?;
    ...
    fn merge_incremental_coverage(&self, new_coverage: CoverageMap) -> Result<CoverageMap> {
        match self.load_coverage_map()? {
            Some(existing_coverage) => self.merge_coverage_maps(&existing_coverage, &new_coverage),
    ...
    fn compute_file_digest(&self, file_path: &Path) -> Result<String> {
        ...
        let mut file =
            std::fs::File::open(file_path).context("Failed to open file for digest computation")?;
    ...
    fn generate_json_report(&self, coverage_map: &CoverageMap) -> Result<()> {
        ...
        std::fs::write(&json_path, json).context("Failed to write JSON coverage report")?;
    ...
    fn generate_lcov_report(&self, coverage_map: &CoverageMap) -> Result<()> {
        ...
        std::fs::write(&lcov_path, lcov_content).context("Failed to write LCOV coverage report")?;
