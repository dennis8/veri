...
impl FlakyManager {
    ...
    pub fn load(&mut self) -> Result<()> {
        if self.database_path.exists() {
            ...
            info!("Loaded flaky database with {} test histories", self.database.test_history.len());
        } else {
    ...
    pub fn execute_test_with_retry<F>(&mut self, nodeid: &str, executor: F) -> Result<TestExecutionResult>
    where
        F: Fn() -> Result<(bool, Duration, Option<String>)>,
    {
        let mut attempt_results = Vec::new();
        let mut final_passed = false;
        let mut final_failure_reason = None;
        
        let max_attempts = if self.config.auto_retry { self.config.retry_count + 1 } else { 1 };
        
        for attempt in 1..=max_attempts {
            info!("Executing test {} (attempt {}/{})", nodeid, attempt, max_attempts);
            ...
            if passed {
                ...
                if attempt < max_attempts {
                    warn!("Test {} failed (attempt {}), retrying in {:?}...", 
    ...
    fn record_test_result(&mut self, nodeid: &str, runs: &[TestRun]) -> Result<()> {
        ...
        self.database.test_history.insert(nodeid.to_string(), history);
    ...
    fn update_flaky_score(&mut self, history: &mut TestHistory) {
        ...
        if history.is_flaky && !was_flaky {
            ...
            warn!("Test {} marked as flaky (score: {:.2})", history.nodeid, history.flaky_score);
        } else if !history.is_flaky && was_flaky && history.consecutive_passes >= 5 {
            // Test has been stable for a while, remove flaky status
            info!("Test {} no longer considered flaky after {} consecutive passes", 
    ...
    fn calculate_recent_failure_rate(&self, recent_runs: &[TestRun]) -> f64 {
        ...
        let recent_failures = recent_runs.iter()
            .take(recent_count)
    ...
    pub fn generate_report(&self) -> FlakyReport {
        let flaky_tests: Vec<_> = self.database.test_history
            .values()
            .filter(|history| history.is_flaky)
            .cloned()
            .collect();
    ...
    fn generate_recommendations(&self, flaky_tests: &[TestHistory]) -> Vec<String> {
        ...
        let timing_related = flaky_tests.iter()
            .filter(|h| h.recent_runs.iter()
                .any(|run| run.failure_reason
                    .as_ref()
        ...
        let highly_flaky = flaky_tests.iter()
            .filter(|h| h.flaky_score > 0.5)
            .count();
        ...
        recommendations.push("📖 More info: https://docs.veri.dev/troubleshooting#flaky-tests".to_string());
...
impl FlakyReport {
    ...
    pub fn print_report(&self, use_color: bool) {
        ...
        if self.total_flaky == 0 {
            ...
            let color = if self.flaky_percentage > 10.0 { red } else { yellow };
            println!("  {}⚠️  Flaky tests: {} ({:.1}%){}", color, self.total_flaky, self.flaky_percentage, reset);
        ...
        if !self.flaky_tests.is_empty() {
            ...
            for (i, test) in sorted_tests.iter().take(10).enumerate() {
                ...
                println!("  {}{}. {} (score: {:.2}, {}/{} failures){}",
                    color, i + 1, test.nodeid, test.flaky_score, 
                    test.total_failures, test.total_runs, reset);
                
                if let Some(last_failure) = test.recent_runs.iter().find(|run| !run.passed) {
                    if let Some(reason) = &last_failure.failure_reason {
                        println!("     Last failure: {}", reason.lines().next().unwrap_or("Unknown"));
                    }
                }
            }
...
mod tests {
    ...
    fn test_test_execution_with_retry() -> Result<()> {
        ...
        let result = manager.execute_test_with_retry("test_example", || {
            attempt_count += 1;
            if attempt_count == 1 {
                Ok((false, Duration::from_millis(100), Some("Flaky failure".to_string())))
            } else {
