...
use crate::python_worker::{TestsIndex, TestNode};
...
impl TestScheduler {
    ...
    pub fn load_timings(&mut self, timings_data: &TimingsData) -> Result<()> {
        ...
        for timing in &timings_data.timings() {
            ...
            self.timings.insert(timing.nodeid.clone(), test_timing.clone());
            
            // Track last failed tests
            if matches!(test_timing.last_outcome, TestOutcome::Failed | TestOutcome::Error) {
    ...
    pub fn schedule_tests(
        ...
        for nodeid in nodeids {
            let test_info = tests_index.tests.iter()
    ...
    fn sort_tests_by_strategy(&self, test_items: &mut Vec<ScheduledTest>) {
    ...
    fn bin_pack_tests(&self, test_items: Vec<ScheduledTest>) -> Result<Vec<TestBatch>> {
        ...
        for test in test_items {
            ...
            let best_worker_idx = workers.iter()
                .enumerate()
                .filter(|(_, w)| {
                    // Check if adding this test would exceed max batch duration
                    w.estimated_duration_ms + test.estimated_duration_ms <= self.config.max_batch_duration_ms
                })
        ...
        workers.sort_by(|a, b| {
            b.contains_last_failed.cmp(&a.contains_last_failed)
    ...
    pub fn get_scheduling_stats(&self, batches: &[TestBatch]) -> SchedulingStats {
        ...
        let max_duration = batches.iter().map(|b| b.estimated_duration_ms).max().unwrap_or(0);
        let min_duration = batches.iter().map(|b| b.estimated_duration_ms).min().unwrap_or(0);
    ...
    pub fn format_explain(&self, batches: &[TestBatch], stats: &SchedulingStats) -> String {
        ...
        output.push(format!("Test Scheduling Plan:"));
        ...
        output.push(format!("  Estimated duration: {:.1}s", stats.total_estimated_duration_ms as f64 / 1000.0));
        output.push(format!("  Load balance ratio: {:.1}%", stats.load_balance_ratio * 100.0));
        
        if stats.batches_with_failed_tests > 0 {
            output.push(format!("  ⚠️  {} workers have last-failed tests (prioritized)", stats.batches_with_failed_tests));
        }

        output.push(String::new());
        output.push(format!("Worker Distribution:"));
        
        for (_i, batch) in batches.iter().enumerate() {
            let duration_sec = batch.estimated_duration_ms as f64 / 1000.0;
            let failed_indicator = if batch.contains_last_failed { " 🔥" } else { "" };
