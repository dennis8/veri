...
use crate::schemas::{ShardsManifest, Shard, ShardTest, ShardingStrategy, TimingsData};
...
impl TestSharder {
    ...
    pub fn split_tests(&self, num_shards: u32, test_selection: Option<&[String]>) -> Result<ShardsManifest> {
        info!("Splitting tests into {} shards using strategy: {:?}", num_shards, self.config.strategy);
        ...
        if filtered_tests.is_empty() {
            ...
            return Ok(ShardsManifest::new(num_shards, self.config.strategy.clone()));
        }
        ...
        let shards = match self.config.strategy {
            ...
            ShardingStrategy::BinPack => self.bin_pack_sharding(&filtered_tests, num_shards, &timings),
            ShardingStrategy::TimingBased => self.timing_based_sharding(&filtered_tests, num_shards, &timings),
        }?;

        // Calculate total estimated duration
        let total_duration: f64 = shards.iter()
        ...
        // Add metadata
        manifest.metadata.insert("python_version".to_string(), serde_json::Value::String(
            std::env::var("PYTHON_VERSION").unwrap_or_else(|_| "unknown".to_string())
        ));
        manifest.metadata.insert("platform".to_string(), serde_json::Value::String(
            std::env::consts::OS.to_string()
        ));

        info!("Generated {} shards with total estimated duration: {:.1}s", 
            manifest.shards.len(), total_duration);
    ...
    pub fn get_shard<'a>(&self, manifest: &'a ShardsManifest, shard_id: u32) -> Result<Option<&'a Shard>> {
        if shard_id >= manifest.total_shards {
            return Err(anyhow::anyhow!(
                "Shard ID {} is out of range (total shards: {})", 
                shard_id, manifest.total_shards
            ));
        }
    ...
    fn round_robin_sharding(&self, tests: &[TestNode], num_shards: u32) -> Result<Vec<Shard>> {
        debug!("Using round-robin sharding for {} tests across {} shards", tests.len(), num_shards);
        ...
        for (i, test) in tests.iter().enumerate() {
            let shard_idx = (i % num_shards as usize) as usize;
    ...
    fn bin_pack_sharding(&self, tests: &[TestNode], num_shards: u32, timings: &HashMap<String, f64>) -> Result<Vec<Shard>> {
        debug!("Using bin-pack sharding for {} tests across {} shards", tests.len(), num_shards);

        // Create test items with estimated durations
        let mut test_items: Vec<(TestNode, f64)> = tests.iter()
            .map(|test| {
                let duration = timings.get(&test.nodeid)
                    .copied()
        ...
        for (test, duration) in test_items {
            ...
            let min_shard_idx = shards.iter()
                .enumerate()
                .min_by(|a, b| a.1.estimated_duration.partial_cmp(&b.1.estimated_duration).unwrap())
                .map(|(i, _)| i)
                .unwrap();
    ...
    fn timing_based_sharding(&self, tests: &[TestNode], num_shards: u32, timings: &HashMap<String, f64>) -> Result<Vec<Shard>> {
        debug!("Using timing-based sharding for {} tests across {} shards", tests.len(), num_shards);

        // Create test items with priorities and durations
        let mut test_items: Vec<(TestNode, f64, u32)> = tests.iter()
            .map(|test| {
                let duration = timings.get(&test.nodeid)
                    .copied()
                    .unwrap_or(1.0);
                ...
                let priority = if test.markers.contains(&"failed".to_string()) { 0 } else { 1 };
    ...
    fn log_balance_stats(&self, manifest: &ShardsManifest) {
        ...
        let durations: Vec<f64> = manifest.shards.iter()
            .map(|s| s.estimated_duration)
            .collect();
        ...
        for shard in &manifest.shards {
            debug!("  Shard {}: {} tests, {:.1}s estimated", 
    ...
    pub fn validate_manifest(&self, manifest: &ShardsManifest) -> Result<()> {
        ...
        if let Ok(tests_index) = worker.collect_tests(&[]) {
            let available_nodeids: std::collections::HashSet<_> = tests_index.tests.iter()
                .map(|t| &t.nodeid)
                .collect();
            ...
            for shard in &manifest.shards {
                for test in &shard.tests {
                    if !available_nodeids.contains(&test.nodeid) {
                        warn!("Manifest contains test not found in current suite: {}", test.nodeid);
                    }
                }
            }
        }
    ...
    pub fn generate_stats_summary(&self, manifest: &ShardsManifest) -> ShardingStats {
        ...
        let durations: Vec<f64> = manifest.shards.iter().map(|s| s.estimated_duration).collect();
        
        let min_duration = durations.iter().copied().fold(f64::INFINITY, f64::min);
        let max_duration = durations.iter().copied().fold(f64::NEG_INFINITY, f64::max);
        let avg_duration = durations.iter().sum::<f64>() / durations.len() as f64;
        
        let balance_ratio = if max_duration > 0.0 { min_duration / max_duration } else { 1.0 };
    ...
    pub fn format_explain(&self, manifest: &ShardsManifest, stats: &ShardingStats) -> String {
        ...
        for shard in &manifest.shards {
            output.push_str(&format!("  Shard {}: {} tests, {:.1}s\n", 
                shard.shard_id, shard.test_count, shard.estimated_duration));
        }
