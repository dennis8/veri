...
class TestPhase13Compatibility:
    ...
    def test_compatibility_matrix_loading(self):
        ...
        result = subprocess.run(
            ["veri", "--compatibility-report"], capture_output=True, text=True
        )
...
"""
            ...
            result = subprocess.run(
                ["veri", "-v"], cwd=temp_dir, capture_output=True, text=True
            )
...
""")
            ...
            assert any(
                ...
                for keyword in ["fallback", "pytest", "compatibility", "plugin"]
            )
...
""")
            ...
            result = subprocess.run(
                ["veri", "--explain"], cwd=temp_dir, capture_output=True, text=True
            )
    ...
    def test_flaky_report_generation(self):
        ...
        result = subprocess.run(
            ["veri", "--flaky-report"], capture_output=True, text=True
        )
...
""")
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                env={**os.environ, "FORCE_PASS": "1"},
            )  # Force pass to avoid actual flakiness in test
...
class TestPhase13MatrixTesting:
    ...
    def test_environment_detection(self):
        ...
        assert any(
            keyword in output.lower()
            for keyword in ["python", "environment", "compatibility"]
        )
...
class TestPhase13CLIIntegration:
    ...
    def test_new_cli_flags_exist(self):
        ...
        result = subprocess.run(["veri", "--help"], capture_output=True, text=True)
...
""")
            ...
            assert any(
                keyword in output for keyword in ["compatibility", "plugin", "security"]
            )
