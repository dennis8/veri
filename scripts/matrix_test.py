...
'''
    ...
    def run_compatibility_test(self, test_dir: Path, test_name: str) -> Dict:
        ...
        try:
            ...
            result.update({
                "success": process.returncode == 0 or test_name in ["help", "version", "explain_mode"],
    ...
    def generate_recommendations(self, results: Dict) -> List[str]:
        ...
        if "version" in failed_tests:
            recommendations.append("⚠️  veri binary may not be properly installed or accessible")
        
        if "basic_run" in failed_tests:
            recommendations.append("⚠️  Basic test execution failed - check veri installation and Python environment")
        
        if "parallel_execution" in failed_tests:
            recommendations.append("⚠️  Parallel execution failed - may be a platform-specific issue")
        
        if "coverage" in failed_tests:
            recommendations.append("⚠️  Coverage collection failed - check if coverage dependencies are installed")
        
        if "pytest_engine" in failed_tests:
            recommendations.append("⚠️  Pytest engine fallback failed - check pytest installation")
        
        # Check for plugin-related issues
        stderr_content = " ".join(r.get("stderr", "") for r in results.values())
        if "plugin" in stderr_content.lower() or "incompatible" in stderr_content.lower():
            recommendations.append("🔌 Plugin compatibility issues detected - consider updating plugins or using --engine pytest")
        
        if len(failed_tests) > len(results) // 2:
            recommendations.append("🚨 More than half of tests failed - this environment may have significant compatibility issues")
        
        recommendations.append("📖 For troubleshooting help: https://docs.veri.dev/troubleshooting")
...
def main():
    ...
    parser.add_argument(
        "--veri-binary", 
        default="veri",
        help="Path to veri binary (default: veri)"
    )
    parser.add_argument(
        "--output",
