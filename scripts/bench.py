...
class BenchmarkRunner:
    ...
    def benchmark_scenario(
        self, name: str, command: List[str], setup_fn=None, teardown_fn=None
    ) -> BenchmarkResult:
    ...
    def make_large_change(self) -> Path:
        ...
        for py_file in self.repo_path.rglob("*.py"):
            ...
            if any(
                keyword in name
                for keyword in ["__init__", "utils", "common", "base", "core"]
            ):
...
class TestSuiteManager:
    ...
    SUITE_CONFIGS = {
        ...
        "polars": {
            ...
            "setup_commands": [["uv", "pip", "install", "py-polars/"]],
        },
        "sqlalchemy": {
...
class PerformanceValidator:
    ...
    def validate_results(self, results: Dict[str, BenchmarkResult]) -> Dict[str, bool]:
        ...
        if "cold_pytest" in results and "cold_veri" in results:
            ...
            validation["collection_speedup"] = (
                speedup >= self.TARGETS["collection_speedup"]
            )
        ...
        if "impact_analysis" in results:
            ...
            validation["impact_analysis"] = (
                analysis_ms <= self.TARGETS["impact_analysis_ms"]
            )
...
class ReportGenerator:
    ...
    def generate_json_report(self, suite: BenchmarkSuite, filename: str):
        ...
        report_data = {
            ...
            "scenarios": {
                name: asdict(result) for name, result in suite.scenarios.items()
            },
    ...
    def generate_markdown_report(self, suite: BenchmarkSuite, filename: str):
        ...
        if "cold_pytest" in scenarios and "cold_veri" in scenarios:
            improvements["cold"] = (
                scenarios["cold_pytest"].mean / scenarios["cold_veri"].mean
            )

        if "hot_pytest" in scenarios and "hot_veri" in scenarios:
            improvements["hot"] = (
                scenarios["hot_pytest"].mean / scenarios["hot_veri"].mean
            )
...
def get_environment_info() -> Dict[str, str]:
    ...
    try:
        result = subprocess.run(
            ["uv", "tool", "run", "veri", "--version"], capture_output=True, text=True
        )
        if result.returncode == 0:
...
def run_complete_benchmark(
    suite_name: str, scenarios: List[str], output_dir: Path, runs: int = 5
):
    ...
    if "cold" in scenarios:
        ...
        results["cold_pytest"] = runner.benchmark_scenario(
            "cold_pytest", ["uv", "run", "pytest", "-q", "--tb=no"]
        )

        # Cold veri
        runner.ensure_clean_environment()
        results["cold_veri"] = runner.benchmark_scenario(
            "cold_veri", ["uv", "tool", "run", "veri", "-a", "-q"]
        )
...
def main():
    ...
    parser.add_argument(
        "--suite",
        choices=["fastapi", "pydantic", "polars", "sqlalchemy"],
