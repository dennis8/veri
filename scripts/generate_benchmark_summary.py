...
def generate_summary_table(
    results: Dict[str, dict], improvements: Dict[str, Dict[str, float]]
) -> str:
...
def generate_detailed_results(results: Dict[str, dict]) -> str:
    ...
    for suite_name in sorted(results.keys()):
        ...
        for scenario_name, scenario_data in scenarios.items():
            ...
            detailed += (
                f"{scenario_data['mean']:.3f}s ± {scenario_data['stdev']:.3f}s\n\n"
            )
        ...
        if "cold_pytest" in scenarios and "cold_veri" in scenarios:
            ...
            detailed += (
                f"**Cold Run**: pytest {pytest_time:.2f}s → veri {veri_time:.2f}s "
            )
        ...
        if "hot_pytest" in scenarios and "hot_veri" in scenarios:
            ...
            detailed += (
                f"**Hot Run**: pytest {pytest_time:.2f}s → veri {veri_time:.2f}s "
            )
            detailed += f"({improvement:.1f}x improvement)\n\n"
...
def generate_performance_summary(results: Dict[str, dict]) -> str:
    ...
    # Overall statistics
    cold_improvements = [
        imp.get("cold_improvement", 0)
        for imp in improvements.values()
        if imp.get("cold_improvement", 0) > 0
    ]
    hot_improvements = [
        imp.get("hot_improvement", 0)
        for imp in improvements.values()
        if imp.get("hot_improvement", 0) > 0
    ]
    ...
    if total_targets > 0:
        ...
        summary += (
            f"**Targets Met**: {met_targets}/{total_targets} ({target_pct:.0f}%)\n\n"
        )
